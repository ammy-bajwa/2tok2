+function($, window){ 'use strict';
	...
	app.setFooterHeight = function(){
		app.$main.css({
			paddingBottom: app.$footer.height() + 'px'
		})
	};
	...
	$('body').on('focus', '.fake-readonly', function () {
		$(this).removeAttr('readonly');
		$(this).css({'background': '#fff'});
	});
	$('body').on('blur', '.fake-readonly', function () {
		$(this).attr('readonly', true);
	});
	$('body').on('focus', '.fake-password-input', function () {
		$(this).attr('type', 'password');
	});

	$('document').ready(function () {
		$('body').find('[data-counterup]').counterUp({
			delay: 10,
			time: 1000
		});
	});
...
+function($, window){ 'use strict';
	...
	var menubar = {
		...
		highlightOpenLink: function() {
			var currentPageName = location.pathname;
			var currentPageHref = location.href;
			var currentPageLink = $appMenu.find('a[href="'+currentPageName+'"]').first();
			var currentPageLinkHref = $appMenu.find('a[href="'+currentPageHref+'"]').first();

			if (currentPageLink.length <= 0){
				currentPageLink = currentPageLinkHref;
			}

			currentPageLink.parents('li').addClass('active');

			if ($body.hasClass('menubar-left') && !this.folded) {
				currentPageLink.parents('.has-submenu').addClass('open').find('>.submenu').show();
			}
...
(function($){
    $(document).on('click', '.showModalButton', function () {
        ...
        if ($('#modal').data('bs.modal').isShown) {
            ...
            document.getElementById('modalHeaderTitle').innerHTML = '<h4 class="modal_header_h4">' + $(this).data('title-modal') + '</h4>';
            ...
            $('body').find('.designed-radio:checked').each(function(){
...
function purchaseRedirect(data){
    Swal.fire({
        title: data.title,
        text: data.text,
        type: data.type
        ...
        //alert('ok');
    	window.location.href = data.url+'?open=1';
    });
}


jQuery('body').on('keydown', '[data-pattern-input]', function (event) {
	let hasSelection = (this.selectionStart !== this.selectionEnd);
	if ((event.ctrlKey || event.metaKey) && (jQuery.inArray(event.keyCode, [67, 86, 88]) > -1)){
		return true;
	}
	let allowedKeyCodesSpecials = [
		8, 9, 13, 16, 17, 18, 35, 36, 37, 38, 39, 40, 45, 46, 225,  //special keys
	];
	let inputObj = jQuery(this);

	let pattern = new RegExp(inputObj.data('pattern-input'), 'gu');
	let str = inputObj.val() + event.key;
	return str.match(pattern) || hasSelection || jQuery.inArray(event.keyCode, allowedKeyCodesSpecials) > -1;
});
...
jQuery(document).ready(function () {
	jQuery('[data-time-convert]').each(function (k, elem) {
        var datetime = new Date(jQuery(elem).data('time-convert'));
        if (datetime instanceof Date && !isNaN(datetime)){
            jQuery(elem).html(convertUTCDateToLocalDate(datetime).toLocaleString());
        }
	});
});